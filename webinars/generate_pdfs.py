#!/usr/bin/env python3
"""
Generate PDF files for all webinar pages using quarto.
This script:
1. Reads each webinar page .qmd file
...
# Template
TEMPLATE_FILE = PAGES_DIR / "_webinar-pdf-template.qmd"
...
def extract_sections(content):
    ...
    # Get webinar overview
    overview_match = re.search(r'## Webinar Overview\s+(.*?)(?=##|\Z)', content, re.DOTALL)
    if overview_match:
        sections['overview'] = overview_match.group(1).strip()
    
    # Get key topics
    topics_match = re.search(r'## Key Topics\s+(.*?)(?=##|\Z)', content, re.DOTALL)
    if topics_match:
        sections['key_topics'] = topics_match.group(1).strip()
    
    # Get agenda if present
    agenda_match = re.search(r'## Agenda\s+(.*?)(?=##|\Z)', content, re.DOTALL)
    if agenda_match:
        sections['agenda'] = agenda_match.group(1).strip()
    
    # Get presenter info
    presenter_match = re.search(r'## About (the Presenter|the Presenters)\s+(.*?)(?=##|\Z)', content, re.DOTALL)
    if presenter_match:
        sections['presenter_info'] = presenter_match.group(2).strip()
...
def main():
    """Main function to process all webinar pages."""
    # Create output directory
    os.makedirs(PDFS_DIR, exist_ok=True)
    
    # Process each webinar page
